...
class HermesCLI:
    ...
    def _smart_route_model(self, user_message: str) -> str:
        """Route the turn to a cheaper model for simple queries.

        Controlled by config.yaml → smart_routing section.  When disabled
        (default) this is a no-op and always returns the primary model.
        """
        cfg = CLI_CONFIG.get("smart_routing", {}) if CLI_CONFIG else {}
        if not cfg.get("enabled"):
            return self.model

        score = self._assess_turn_difficulty(user_message)
        tiers = cfg.get("tiers", {})

        if score < 0.3:
            return tiers.get("cheap", self.model)
        if score < 0.7:
            return tiers.get("balanced", self.model)
        return self.model

    @staticmethod
    def _assess_turn_difficulty(user_message: str) -> float:
        """Heuristic difficulty score (0.0–1.0) – zero-cost, zero-latency."""
        ...
        # ── simple intent keywords → cap low ────────────────────
        _simple = (
            "hello", "hi ", "help", "what is", "who is", "when ",
            "where ", "how do i", "explain", "define", "list ",
            "weather", "time ", "date ", "translate",
        )
        if any(kw in msg for kw in _simple):
            score = min(score, 0.25)

        # ── complex intent keywords → floor high ─────────────────
        _complex = (
            "refactor", "debug", "implement", "architecture",
            "design ", "optimize", "migrate", "deploy",
            "investigate", "troubleshoot", "analyze",
            "generate ", "build ", "rewrite",
        )
        if any(kw in msg for kw in _complex):
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        Uses Smart Routing when enabled; otherwise falls back to the
        session's primary model/provider.  If the user has toggled `/fast`
        ...
        effective_model = self._smart_route_model(user_message)
        route = {
