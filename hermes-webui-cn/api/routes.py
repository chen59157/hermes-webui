...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/platforms":
        from api.config import save_platform_config
        try:
...
def _handle_chat_start(handler, body, diag=None):
    try:
        ...
        try:
            ...
            return bad(handler, str(e))
        user_explicit_model = body.get("model")
        user_explicit_provider = body.get("model_provider")
        requested_model = user_explicit_model or s.model
        requested_provider = (
            user_explicit_provider
            if "model_provider" in body
            else getattr(s, "model_provider", None)
        )
        ...
        # When the user explicitly picked a model in the UI dropdown (model came
        # from the request body, not a stale session fallback), honour that
        # choice even if the normalization logic detected a cross-provider
        # mismatch.  _resolve_compatible_session_model_state is designed to
        # repair stale persisted session models, not to override live UI
        # selections.  (#model-switch-fix)
        if normalized_model and user_explicit_model:
            logger.info(
                "Skipping model normalization for explicit user selection: "
                "requested=%r provider=%r  normalized_to=%r norm_provider=%r",
                user_explicit_model, user_explicit_provider,
                model, model_provider,
            )
            model = user_explicit_model
            model_provider = user_explicit_provider or requested_provider
