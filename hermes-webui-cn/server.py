...
# ── 预导入 asyncio，确保在复杂 import 链中最先完成加载 ─────────────────
# cron.scheduler < hermes_cli.plugins 等多个模块均在模块级 import asyncio，
# 若第一个 import asyncio 发生在嵌套较深的 import 链中且被其他模块的
# 并行 import 打断，可能出现 base_events 未定义的竞态问题。
# 在 server.py 最顶部预先完成 asyncio 加载可避免此问题。
import asyncio  # noqa: F401 – preloaded to avoid import race

# ── No-console subprocess patch (must run before any agent import) ──────────
# 消除 Agent 引擎内部 subprocess 调用时弹出的 cmd 终端窗口。
# 由 hermes_desktop.py 通过 HERMES_SUBPROCESS_NO_WINDOW=1 环境变量控制。
try:
    from no_console_patch import apply_patch
    apply_patch()
except Exception:
    pass
