...
async function loadSession(sid){
  ...
  if(INFLIGHT[sid]){
    ...
    if(activeStreamId){
      ...
      // 恢复最后一条助手消息的思考内容到右侧面板
      if(typeof restoreThinkingPanel==='function'){
        const lastAsst=[...S.messages].reverse().find(m=>m&&m.role==='assistant'&&m.reasoning);
        if(lastAsst) restoreThinkingPanel(lastAsst.reasoning);
        else clearThinkingPanel();
      }
...
let _serverTz = '';
let _groupCollapsedCache = null;

function filterSessions(){
  renderSessionListFromCache();
}

function _highlightSearchMatch(text, query) {
...
function upsertActiveSessionForLocalTurn({title='', messageCount=0, timestampMs=Date.now()}={}){
  ...
  // Defer sidebar rebuild to avoid blocking the main thread before agent API call.
  // Data is already synced; visual update can wait for next idle frame.
  requestAnimationFrame(()=>renderSessionListFromCache());
}
...
// Lightweight active-session highlight update — avoids full sidebar rebuild
// on session switch. Only toggles .active class and unread state.
function _updateSidebarActiveSession(newSid){
  const list = $('sessionList');
  if(!list||!newSid) return;
  const prevActive = list.querySelector('.session-item.active');
  if(prevActive){
    const prevSid = prevActive.dataset.sid;
    prevActive.classList.remove('active','new-flash');
    // Refresh unread state for previously-active session
    if(prevSid&&typeof _hasUnreadForSession==='function'){
      const prevSession = _allSessions?.find(s=>s.session_id===prevSid);
      if(prevSession) prevActive.classList.toggle('unread',_hasUnreadForSession(prevSession));
    }
  }
  const newActive = list.querySelector(`.session-item[data-sid="${CSS.escape(newSid)}"]`);
  if(newActive){
...
function renderSessionListFromCache(){
  ...
  // Collapse state — cached; invalidates on write
  if(_groupCollapsedCache===null){
    try{_groupCollapsedCache=JSON.parse(localStorage.getItem('hermes-date-groups-collapsed')||'{}');}catch(e){_groupCollapsedCache={};}
  }
  const _groupCollapsed=_groupCollapsedCache;
  const _saveCollapsed=()=>{
    try{localStorage.setItem('hermes-date-groups-collapsed',JSON.stringify(_groupCollapsed));}catch(e){}
    _groupCollapsedCache=null; // invalidate on next render
  };
  ...
  function _renderOneSession(s, isPinnedGroup=false){
    ...
    if(q&&cleanTitle&&cleanTitle!=='Untitled'){
      title.innerHTML=_highlightSearchMatch(cleanTitle,q);
    } else {
    ...
    el.onpointerup=(e)=>{
      ...
      _tapTimer=setTimeout(async()=>{
        ...
        await loadSession(s.session_id);_updateSidebarActiveSession(s.session_id);
