...
// ── Thinking Panel (右侧活动面板思考过程) ──────────────────────────────────
function updateThinkingPanel(text){
  const panel=document.getElementById('thinkingPanel');
  const content=document.getElementById('thinkingPanelContent');
  const cursor=document.getElementById('thinkingPanelCursor');
  ...
  if(header&&!header.querySelector('.thinking-panel-dot')){
    ...
    dot.className='thinking-panel-dot';
    header.insertBefore(dot,header.querySelector('.thinking-panel-title'));
  }
  // 自动滚动到底部
...
function restoreThinkingPanel(reasoning){
...
function ensureActivityGroup(inner, opts){
  console.log('[DIAG] ensureActivityGroup called: inner=', !!inner, 'opts=', opts);
  opts=opts||{};
...
function _postRenderBatch(container){
  // Skip heavy work when the container has no relevant content.
  // highlightCode (Prism) and addCopyButtons are the main cost.
  const hasCode=container.querySelector('pre > code, code[class*="language-"]');
  if(hasCode){
    highlightCode(container);
    addCopyButtons(container);
  }
  // The rest have their own [data-loaded] / .diff-inline-load guards.
  loadDiffInline();
  loadCsvInline();
...
function renderMessages(options){
  ...
  if(sid&&sid!==_sessionHtmlCacheSid&&!INFLIGHT[sid]&&!hasTransientTranscriptUi){
    ...
    if(cached&&cached.msgCount===msgCount&&cached.renderWindowSize===renderWindowSize){
      ...
      requestAnimationFrame(()=>{_postRenderBatch(inner);});
  ...
  for(let vi=0;vi<renderVisWithIdx.length;vi++){
    ...
    const footHtml = `<div class="msg-foot">${timeHtml}<span class="msg-actions">${editBtn}${ttsBtn}${forkBtn}${retryBtn}</span></div>`;
...
function appendLiveToolCard(tc){
  console.log('[DIAG] appendLiveToolCard called: tc=', tc);
...
// --- Rollback Panel ---
async function showRollbackPanel(){
  var panel=document.getElementById('rollbackPanel');
  var body=document.getElementById('rollbackPanelBody');
  if(!panel||!body) return;
  panel.style.display='';
  body.innerHTML='<div class="rollback-loading">加载检查点...</div>';
  try{
    var ws=S.session?(S.session.workspace||''):'';
    if(!ws){body.innerHTML='<div class="rollback-empty">没有活动工作区</div>';return;}
    var data=await api('/api/rollback/list?workspace='+encodeURIComponent(ws));
    var cks=data.checkpoints||[];
    if(!cks.length){body.innerHTML='<div class="rollback-empty">暂无检查点</div>';return;}
    var h='';
    for(var ci=0;ci<cks.length;ci++){
      var ck=cks[ci];
      h+='<div class="rollback-item"><div class="rollback-item-info">';
      h+='<div class="rollback-item-msg">'+esc(ck.message||ck.id)+'</div>';
      h+='<div class="rollback-item-meta">'+esc(ck.date_display||'')+' - '+ck.files+' files - '+esc((ck.commit||'').slice(0,7))+'</div>';
      h+='</div><div class="rollback-item-actions">';
      h+='<button class="rollback-btn" onclick="event.stopPropagation();_viewRollbackDiff(this.getAttribute(\'data-ws\'),this.getAttribute(\'data-ck\'))" data-ws="'+esc(ws)+'" data-ck="'+esc(ck.id)+'">Diff</button>';
      h+='<button class="rollback-btn restore" onclick="event.stopPropagation();_doRollbackRestore(this.getAttribute(\'data-ws\'),this.getAttribute(\'data-ck\'),this.getAttribute(\'data-msg\'))" data-ws="'+esc(ws)+'" data-ck="'+esc(ck.id)+'" data-msg="'+esc(ck.message||ck.id)+'">Restore</button>';
      h+='</div></div>';
    }
    body.innerHTML=h;
  }catch(e){body.innerHTML='<div class="rollback-empty">Error: '+esc(e.message)+'</div>';}
}
function hideRollbackPanel(){var p=document.getElementById('rollbackPanel');if(p)p.style.display='none';}
async function _viewRollbackDiff(ws,ckId){
  try{
    var d=await api('/api/rollback/diff?workspace='+encodeURIComponent(ws)+'&checkpoint='+encodeURIComponent(ckId));
    alert((d.diff||'No diff').substring(0,2000));
  }catch(e){alert('Error: '+e.message);}
}
async function _doRollbackRestore(ws,ckId,msg){
  if(!confirm('Restore to "'+msg+'"?\n\nThis will overwrite current files.'))return;
  try{
    var d=await api('/api/rollback/restore',{method:'POST',body:JSON.stringify({workspace:ws,checkpoint:ckId})});
    if(d.ok){if(typeof showToast==='function')showToast('Restored: '+msg,3000,'success');hideRollbackPanel();}
...
(function(){
  ...
  if(!msgEl) return;
  var atActive=false,atIdx=0,atItems=[],atTimer=null;

  function showAt(items){
    var popup=document.getElementById('atAutocomplete'),list=document.getElementById('atAutocompleteList');
    if(!popup||!list) return;
    if(!items.length){popup.style.display='none';atActive=false;return;}
    atItems=items;atIdx=0;atActive=true;
    var h='';
    for(var i=0;i<items.length;i++){
      var it=items[i];
      h+='<div class="at-item'+(i===0?' active':'')+'" data-idx="'+i+'">'+(it.is_dir?'Folder':'File')+' <span class="at-item-name">'+esc(it.name)+'</span></div>';
    }
    list.innerHTML=h;
    popup.style.display='';
    var els=list.querySelectorAll('.at-item');
    for(var j=0;j<els.length;j++){
      (function(el){el.onclick=function(){selAt(parseInt(el.dataset.idx));};})(els[j]);
    }
  }
  function hideAt(){var p=document.getElementById('atAutocomplete');if(p)p.style.display='none';atActive=false;}
  function selAt(idx){
    if(idx<0||idx>=atItems.length) return;
    var it=atItems[idx],val=msgEl.value,pos=msgEl.selectionStart,atPos=val.lastIndexOf('@',pos);
    if(atPos===-1) return;
    var ins=(it.is_dir?it.name+'/':it.name)+' ';
    msgEl.value=val.slice(0,atPos)+'@'+ins+val.slice(pos);
    msgEl.selectionStart=msgEl.selectionEnd=atPos+1+ins.length;
    msgEl.focus();hideAt();
  }
  function updHL(){
    var l=document.getElementById('atAutocompleteList');if(!l)return;
    var els=l.querySelectorAll('.at-item');
    for(var i=0;i<els.length;i++) els[i].classList.toggle('active',i===atIdx);
    var a=l.querySelector('.at-item.active');if(a)a.scrollIntoView({block:'nearest'});
  }
  async function fetchF(q){
    try{
      var sid=S.session?S.session.session_id:'';
      if(!sid) return [];
      var d=await api('/api/list?session_id='+encodeURIComponent(sid)+'&path=.');
      var es=d.entries||[],lq=q.toLowerCase(),out=[];
      for(var i=0;i<es.length;i++){
        if(es[i].name&&es[i].name.toLowerCase().indexOf(lq)!==-1){
          out.push({name:es[i].name,is_dir:es[i].is_dir===true||es[i].type==='dir'});
  ...
  msgEl.addEventListener('input',function(){
    var pos=this.selectionStart,val=this.value.slice(0,pos),m=val.match(/@([^\s@]*)$/);
    if(m){clearTimeout(atTimer);atTimer=setTimeout(async function(){showAt(await fetchF(m[1]));},200);}
    else hideAt();
  });
  msgEl.addEventListener('keydown',function(e){
    if(!atActive) return;
    if(e.key==='ArrowDown'){e.preventDefault();atIdx=Math.min(atIdx+1,atItems.length-1);updHL();}
    else if(e.key==='ArrowUp'){e.preventDefault();atIdx=Math.max(atIdx-1,0);updHL();}
    else if(e.key==='Enter'||e.key==='Tab'){if(atItems.length){e.preventDefault();selAt(atIdx);}}
    else if(e.key==='Escape') hideAt();
  });
  document.addEventListener('click',function(e){
    if(atActive&&!e.target.closest('.at-autocomplete')&&!e.target.closest('#msg'))hideAt();
  });
})();
