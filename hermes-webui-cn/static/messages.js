...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  // 清空右侧思考面板（新消息开始）
  if(typeof clearThinkingPanel==='function') clearThinkingPanel();
  ...
  // Incremental sanitise state: avoids re-scanning already-checked anchors/images.
  let _smdLastSanitisedA=0, _smdLastSanitisedImg=0;
  ...
  function _smdNewParser(el){
    _smdWrittenLen=0;
    _smdWrittenText='';
    _smdLastSanitisedA=0;
    _smdLastSanitisedImg=0;
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('reasoning',e=>{
      ...
      // 同步更新右侧思考面板
      if(typeof updateThinkingPanel==='function') updateThinkingPanel(reasoningText);
      _scheduleRender();
    });

    source.addEventListener('tool',e=>{
      const d=JSON.parse(e.data);
      console.log('[DIAG] SSE tool event received:', JSON.stringify(d));
    ...
    source.addEventListener('tool_complete',e=>{
      const d=JSON.parse(e.data);
      console.log('[DIAG] SSE tool_complete event received:', JSON.stringify(d));
      if(d.name==='clarify') return;
      const inflight=INFLIGHT[activeSid];
      if(!inflight) return;
      if(!Array.isArray(inflight.toolCalls)) inflight.toolCalls=[];
    ...
    source.addEventListener('approval',e=>{
      ...
      _forceNativeToast('⚠️ 需要授权',d.description||'Agent 请求执行需要确认的操作');
    });
    ...
    source.addEventListener('clarify',e=>{
      ...
      playNotificationSound();
      _forceNativeToast('💬 需要回复',d.question||'Agent 需要您提供更多信息');
    });
    ...
    source.addEventListener('title',e=>{
      ...
      if(typeof renderSessionListFromCache==='function') requestAnimationFrame(()=>renderSessionListFromCache());
    ...
    source.addEventListener('done',e=>{
      ...
      if(isActiveSession){
        ...
        loadDir('.');
        // 标记思考面板为完成状态
        if(typeof finalizeThinkingPanel==='function') finalizeThinkingPanel();
...
function sendBrowserNotification(title,body){
  ...
  // 优先使用 Windows 原生 Toast 通知（通过 pywebview JSBridge）
  try{
    if(window.pywebview&&window.pywebview.api&&typeof window.pywebview.api.show_toast==='function'){
      window.pywebview.api.show_toast(title||'Hermes',body||'');
      return;
    }
  }catch(e){}
  // 降级：Web Notification API（仅在窗口隐藏时）
...
// 强制弹出原生 Toast（不受 notifications_enabled 设置限制）
// 用于权限请求等必须引起用户注意的场景
function _forceNativeToast(title,body){
  try{
    if(window.pywebview&&window.pywebview.api&&typeof window.pywebview.api.show_toast==='function'){
      window.pywebview.api.show_toast(title||'Hermes',body||'');
      return;
  ...
  // 降级：Web Notification API
  try{
    if('Notification' in window&&Notification.permission==='granted'){
      new Notification(title||'Hermes',{body:body||''});
    }else if('Notification' in window&&Notification.permission!=='denied'){
      Notification.requestPermission().then(p=>{
        if(p==='granted') new Notification(title||'Hermes',{body:body||''});
      });
    }
  }catch(e){}
}
