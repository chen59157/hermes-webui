...
function _setWorkspacePanelMode(mode){
  ...
  // When opening panel, also set the pref so it stays open after refresh
  if(mode==='browse'||mode==='preview'){
    localStorage.setItem('hermes-webui-workspace-panel-pref','open');
  }
  layout.classList.toggle('workspace-panel-collapsed',!open);
  if(open && typeof switchRightPanelTab === 'function') switchRightPanelTab('activity');
...
function closeWorkspacePanel(){
  localStorage.removeItem('hermes-webui-workspace-panel-pref');
...
(async()=>{
  ...
  await renderSessionList();
  _initResizePanels();
  // 强制恢复右侧面板（确保 Agent 运行状态可见）
  (function() {
    const pref = localStorage.getItem('hermes-webui-workspace-panel-pref');
    const runtime = localStorage.getItem('hermes-webui-workspace-panel');
    if (pref !== 'open' && runtime !== 'open') {
      localStorage.setItem('hermes-webui-workspace-panel', 'open');
      localStorage.setItem('hermes-webui-workspace-panel-pref', 'open');
      console.log('[Hermes] 已强制恢复右侧面板偏好');
    }
    // 立即设置面板模式为 'browse'，确保面板在 UI 初始化时打开
    _setWorkspacePanelMode('browse');
    // 默认显示 Agent 活动面板
    switchRightPanelTab('activity');
  })();
...
// ============================================================
(function(){
  const elSearch = document.getElementById('conversationSearch');
  const elInput = document.getElementById('convSearchInput');
  const elCount = document.getElementById('convSearchCount');
  const elPrev = document.getElementById('convSearchPrev');
  const elNext = document.getElementById('convSearchNext');
  const elClose = document.getElementById('convSearchClose');
  const elCollapse = document.getElementById('convSearchCollapse');
  const elExpand = document.getElementById('convSearchExpand');
  const elMessages = document.getElementById('messages');
  if(!elSearch||!elInput||!elMessages) return;
  ...
  function getCollapsed(){ return _collapsed; }
  function setCollapsed(v){
    _collapsed=v;
    elSearch.classList.toggle('collapsed',v);
    if(v){ elInput.blur(); clearHighlights(); }
    else { setTimeout(function(){ elInput.focus(); }, 150); }
  }
  window._convSearchGetCollapsed = getCollapsed;
  window._convSearchSetCollapsed = setCollapsed;
  ...
  function doSearch(){
    ...
    clearHighlights();
    if(!q){ elCount.textContent=''; elPrev.disabled=true; elNext.disabled=true; return; }

    // Collect all text-bearing message elements
    const msgEls = elMessages.querySelectorAll('.msg-row');
    msgEls.forEach(function(msg){
      // Skip system/thinking rows, only search user/assistant content
      const contentEl = msg.querySelector('.msg-body') || msg;
      ...
      while((pos = lower.indexOf(qLower, pos)) !== -1){
        _results.push({ el: msg, start: pos, length: q.length });
        pos += qLower.length;
      }
    });

    const total = _results.length;
    elCount.textContent = total ? `0/${total}` : '0/0';
    elPrev.disabled = total === 0;
    elNext.disabled = total === 0;
    if(total > 0){ goTo(0); }
  }
  ...
  function goTo(idx){
    ...
    _idx = idx;
    // Clear only the one active highlight — much faster than querySelectorAll
    if(_activeHighlight){
      const p = _activeHighlight.parentNode;
      if(p){ p.replaceChild(document.createTextNode(_activeHighlight.textContent), _activeHighlight); }
      _activeHighlight = null;
    }
    const r = _results[idx];
    const contentEl = r.el.querySelector('.msg-body') || r.el;
    if(contentEl){
      try{ _activeHighlight = highlightText(contentEl, r.start, r.length); } catch(e){}
    }
    // Scroll into view — always execute even if highlighting failed
    r.el.scrollIntoView({block:'center', behavior:'smooth'});
    elCount.textContent = `${idx+1}/${_results.length}`;
  }

  function clearHighlights(){
    // Bulk clear: fallback when closing search or doing a fresh search
    const all = elMessages.querySelectorAll('.message-highlight');
    for(const el of all){
  ...
  function highlightText(node, start, length){
    const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, null, false);
    let offset = 0;
    const endChar = start + length;
    let firstSpan = null;
    while(walker.nextNode()){
      const tn = walker.currentNode;
      const nodeLen = tn.textContent.length;
      const nodeEnd = offset + nodeLen;
      if(nodeEnd <= start || offset >= endChar){ offset = nodeEnd; continue; }
      const localStart = Math.max(0, start - offset);
      const localEnd = Math.min(nodeLen, endChar - offset);
      if(localStart >= localEnd){ offset = nodeEnd; continue; }
  ...
  // Expand from collapsed bar
  elExpand.addEventListener('click', function(){
    setCollapsed(false);
  });

  // Keyboard shortcut: Ctrl+Shift+F to open/expand search
  document.addEventListener('keydown', function(e){
    ...
    if(e.key==='Escape' && document.activeElement === elInput && !elInput.value){
      setCollapsed(true);
    }
  });
  ...
  elInput.addEventListener('keydown', function(e){
    if(e.key==='Enter'){ e.preventDefault(); if(_results.length) { _idx = _idx < _results.length-1 ? _idx+1 : 0; goTo(_idx); } }
  });

  elPrev.addEventListener('click', function(){ if(_results.length){ _idx = _idx > 0 ? _idx-1 : _results.length-1; goTo(_idx); } });
  elNext.addEventListener('click', function(){ if(_results.length){ _idx = _idx < _results.length-1 ? _idx+1 : 0; goTo(_idx); } });
...
let _agentActivityTurnId = null;
let _agentActivityItems = [];

function switchRightPanelTab(tab) {
  // 如果面板处于折叠状态，点击标签时展开面板
  if(_workspacePanelMode==='closed'&&typeof toggleWorkspacePanel==='function'){
    toggleWorkspacePanel(true);
  }
  document.querySelectorAll('.rightpanel-tab').forEach(t => t.classList.toggle('active', t.dataset.rpTab === tab));
  document.querySelectorAll('.rightpanel-view').forEach(v => v.classList.toggle('active', v.dataset.rpView === tab));
  // Show/hide workspace-specific header buttons
  const wsBtns = ['btnUpDir','btnNewFile','btnNewFolder','btnRefreshPanel','btnWorkspacePrefs','btnClearPreview'];
  wsBtns.forEach(id => { const el = document.getElementById(id); if (el) el.style.display = tab === 'workspace' ? '' : 'none'; });
  // Toggle workspace hidden indicator
  const wi = document.getElementById('workspaceHiddenIndicator');
  if (wi) wi.style.display = tab === 'workspace' ? '' : 'none';
}

function _setAgentStatus(state, label) {
  const dot = document.getElementById('agentStatusDot');
  const lbl = document.getElementById('agentStatusLabel');
  if (!dot || !lbl) return;
  dot.className = 'agent-status-indicator ' + state;
  lbl.textContent = label;
}

function _addAgentActivityItem(type, label, content) {
  const feed = document.getElementById('agentActivityFeed');
  const empty = document.getElementById('agentActivityEmpty');
  if (!feed) return;
  ...
  const truncatedContent = content && String(content).length > 200 ? String(content).slice(0, 200) + '…' : (content || '');
  const escapedContent = truncatedContent.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');

  item.className = 'agent-activity-item ' + (type === 'thinking' ? 'thinking-item' : 'tool-item');
  item.innerHTML =
    '<div class="agent-activity-item-header" onclick="this.parentElement.classList.toggle(\'open\')">' +
      '<span class="agent-activity-item-icon">' + iconSvg + '</span>' +
      '<span class="agent-activity-item-name">' + esc(label) + '</span>' +
      '<span class="agent-activity-item-time">' + time + '</span>' +
      '<span class="agent-activity-item-toggle"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></span>' +
    '</div>' +
    (escapedContent ? '<div class="agent-activity-item-body"><pre>' + escapedContent + '</pre></div>' : '');

  feed.insertBefore(item, feed.firstChild);
  _agentActivityItems.push({ type: type, label: label, el: item });
  // Keep max 50 items
  while (_agentActivityItems.length > 50) {
    const old = _agentActivityItems.shift();
    if (old && old.el && old.el.parentNode) old.el.remove();
  }
}

function _updateLastAgentActivityItem(content) {
  if (_agentActivityItems.length === 0) return;
  const last = _agentActivityItems[_agentActivityItems.length - 1];
  if (!last || !last.el) return;
  const body = last.el.querySelector('.agent-activity-item-body');
  if (!body) return;
  const pre = body.querySelector('pre');
  if (!pre) return;
  const truncatedContent = content && String(content).length > 800 ? String(content).slice(0, 800) + '…' : (content || '');
  pre.textContent = truncatedContent;
}

function _clearAgentActivity() {
  const feed = document.getElementById('agentActivityFeed');
  if (feed) {
    feed.innerHTML = '<div class="agent-activity-empty" id="agentActivityEmpty">发送消息后，Agent 的思考过程和工具调用将在此显示</div>';
  }
  _agentActivityItems = [];
  _agentActivityTurnId = null;
  _setAgentStatus('idle', '就绪');
}

// Hook into the thinking/tool rendering pipeline
(function() {
  // Defer until ui.js has defined the original functions
  const _hook = setInterval(function() {
    if (typeof appendThinking !== 'function' || typeof appendLiveToolCard !== 'function') return;
    clearInterval(_hook);

    const _origAppendThinking = appendThinking;
    const _origUpdateThinking = updateThinking;
    const _origRemoveThinking = removeThinking;
    const _origFinalizeThinkingCard = finalizeThinkingCard;
    const _origAppendLiveToolCard = appendLiveToolCard;

    appendThinking = function(text) {
      _setAgentStatus('thinking', '思考中…');
      if (text && String(text).trim()) {
        if (_agentActivityItems.length > 0 && _agentActivityItems[_agentActivityItems.length - 1].type === 'thinking') {
          _updateLastAgentActivityItem(text);
        } else {
          _addAgentActivityItem('thinking', 'Agent 思考', text);
        }
      }
      return _origAppendThinking.call(this, text);
    };
    ...
    updateThinking = function(text) {
      ...
      return _origUpdateThinking.call(this, text);
    };

    removeThinking = function() {
      _setAgentStatus('idle', '就绪');
      return _origRemoveThinking.call(this);
    };

    finalizeThinkingCard = function() {
      _setAgentStatus('running', '执行中…');
      return _origFinalizeThinkingCard.call(this);
    };

    appendLiveToolCard = function(tc) {
      _setAgentStatus('running', '执行中…');
      if (tc && tc.name) {
        _addAgentActivityItem('tool', '工具: ' + tc.name, tc.input ? JSON.stringify(tc.input, null, 2) : '');
      }
      return _origAppendLiveToolCard.call(this, tc);
    };
  }, 50);

  // Clear activity panel on new send
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Enter' && !e.shiftKey && document.activeElement && document.activeElement.id === 'composer') {
      _clearAgentActivity();
    }
  });
})();
