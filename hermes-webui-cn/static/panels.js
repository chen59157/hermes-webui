...
async function switchPanel(name, opts = {}) {
  ...
  // Hide right panel (Activity/Workspace) on non-chat views
  if (nextPanel !== 'chat' && typeof closeWorkspacePanel === 'function') {
    closeWorkspacePanel();
  }
...
function _renderSystemHealthPanel() {
  return `
    <section class="insights-card system-health-panel loading" id="systemHealthPanel" aria-label="主机资源健康" aria-live="polite">
      ...
      <div class="system-health-metrics">
        <div class="system-health-metric" data-system-health-metric="cpu">
          ...
          <div class="system-health-bar" role="progressbar" aria-label="CPU 使用率" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="system-health-bar-fill"></div></div>
        </div>
        <div class="system-health-metric" data-system-health-metric="memory">
          <div class="system-health-label"><span>内存</span><span class="system-health-value" data-system-health-value>—</span></div>
          <div class="system-health-bar" role="progressbar" aria-label="内存使用率" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="system-health-bar-fill"></div></div>
        </div>
        <div class="system-health-metric" data-system-health-metric="disk">
          <div class="system-health-label"><span>磁盘</span><span class="system-health-value" data-system-health-value>—</span></div>
          <div class="system-health-bar" role="progressbar" aria-label="磁盘使用率" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="system-health-bar-fill"></div></div>
        </div>
      </div>
...
function switchSettingsSection(name){
  const section=(name==='appearance'||name==='preferences'||name==='providers'||name==='plugins'||name==='assistant'||name==='system')?name:'conversation';
  _settingsSection=section;
  _currentSettingsSection=section;
  const map={conversation:'Conversation',appearance:'Appearance',preferences:'Preferences',providers:'Providers',plugins:'Plugins',assistant:'Assistant',system:'System'};
  // Sidebar menu items
  document.querySelectorAll('#settingsMenu .side-menu-item').forEach(it=>{
    it.classList.toggle('active', it.dataset.settingsSection===section);
  });
  // Panes in main
  ['conversation','appearance','preferences','providers','plugins','assistant','system'].forEach(key=>{
...
document.addEventListener('click',function(e){
  ...
  const titleEl=card.querySelector('.assistant-card-title');
  const chName=titleEl?titleEl.textContent:channel;
  if(btn.classList.contains('config')){
    openPlatformConfig(channel, chName);
    return;
  }
  if(btn.classList.contains('unbind')){
    if(!confirm(`确定要解除 ${chName} 的绑定吗？此操作将断开该通道连接。`)) return;
    const badge=card.querySelector('.assistant-status-badge');
    if(badge){badge.textContent='未连接';badge.classList.remove('connected');badge.classList.add('disconnected');}
    const unbindBtn=card.querySelector('.assistant-btn.unbind');
    if(unbindBtn) unbindBtn.style.display='none';
    // 清除该平台的配置
    fetch('/api/platforms',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',
      body:JSON.stringify({channel,data:{}})}).catch(()=>{});
  }
});
/* ── 平台配置弹窗 ── */
const PLATFORM_FIELDS={
  feishu:[
    ...
    {key:'app_secret',label:'App Secret',placeholder:'xxxxxxxxxx',type:'password'},
    {key:'webhook_url',label:'Webhook URL（可选）',placeholder:'https://open.feishu.cn/...'}
  ],
  'wechat-assistant':[
    {key:'app_id',label:'App ID',placeholder:'wx1234567890abcdef'},
    {key:'app_secret',label:'App Secret',placeholder:'xxxxxxxxxx',type:'password'},
    {key:'token',label:'Token',placeholder:'用于消息验证'}
  ],
  'wechat-service':[
    {key:'corp_id',label:'Corp ID',placeholder:'ww1234567890abcdef'},
    {key:'secret',label:'Secret',placeholder:'xxxxxxxxxx',type:'password'},
    {key:'token',label:'Token',placeholder:'用于消息验证'}
  ],
  wework:[
    {key:'corp_id',label:'Corp ID',placeholder:'ww1234567890abcdef'},
  ...
  'qq-bot':[
    ...
    {key:'token',label:'Bot Token',placeholder:'xxxxxxxxxx',type:'password'}
  ],
  yuanbao:[
    {key:'api_key',label:'API Key',placeholder:'sk-xxxxxxxxxx',type:'password'}
  ],
  dingtalk:[
    {key:'app_key',label:'App Key',placeholder:'dingxxxxxxxxxx'},
    {key:'app_secret',label:'App Secret',placeholder:'xxxxxxxxxx',type:'password'},
    {key:'robot_code',label:'Robot Code（可选）',placeholder:'xxxxxxxxxx'}
  ]
};
let _currentPlatformChannel=null;
function openPlatformConfig(channel, chName){
  _currentPlatformChannel=channel;
  const fields=PLATFORM_FIELDS[channel]||[];
  const container=document.getElementById('platformConfigFields');
  const title=document.getElementById('platformConfigTitle');
  const error=document.getElementById('platformConfigError');
  if(title) title.textContent=(chName||channel)+' — 配置';
  if(error) error.textContent='';
  if(!container) return;
  // 动态生成表单字段
  container.innerHTML=fields.map(f=>`<div class="kanban-modal-row"><label for="pf_${f.key}">${f.label}</label><input type="${f.type||'text'}" id="pf_${f.key}" placeholder="${f.placeholder||''}" autocomplete="off"></div>`).join('');
  // 加载已有配置
  fetch('/api/platforms',{credentials:'include'}).then(r=>r.json()).then(configs=>{
...
function closePlatformConfigModal(){
  const modal=document.getElementById('platformConfigModal');
  if(modal) modal.hidden=true;
  _currentPlatformChannel=null;
}
function submitPlatformConfig(){
  const channel=_currentPlatformChannel;
  if(!channel) return;
  const fields=PLATFORM_FIELDS[channel]||[];
  const data={};
  let hasValue=false;
  ...
  const error=document.getElementById('platformConfigError');
  if(!hasValue){
    if(error) error.textContent='请至少填写一个配置项';
    return;
  }
  if(error) error.textContent='';
  fetch('/api/platforms',{
    ...
    if(result.ok){
      // 更新卡片状态
      const card=document.querySelector(`.assistant-card[data-channel="${channel}"]`);
      if(card){
        const badge=card.querySelector('.assistant-status-badge');
        if(badge){badge.textContent='已配置';badge.classList.remove('disconnected');badge.classList.add('connected');}
      }
      closePlatformConfigModal();
    }else{
      if(error) error.textContent=result.error||'保存失败';
    }
  }).catch(e=>{
    if(error) error.textContent='网络错误: '+e.message;
  });
}
function _loadPlatformStatus(){
  fetch('/api/platforms',{credentials:'include'}).then(r=>r.json()).then(configs=>{
    Object.keys(configs).forEach(channel=>{
      const cfg=configs[channel];
